...
The \textbf{Online Sales Dataset}\footnote{\url{https://www.kaggle.com/datasets/yusufdelikkaya/online-sales-dataset}} is a sample e-commerce database.
...
\begin{itemize}
    \item \textbf{Transaction details}: \texttt{InvoiceNo}, \texttt{InvoiceDate}, \texttt{CustomerID}
    \item \textbf{Product information}: \texttt{StockCode}, \texttt{Description}, \texttt{Category}, \texttt{UnitPrice}, \texttt{Quantity}, \texttt{Discount}
...
\begin{itemize}
    ...
    \item \textbf{Consistency Problems}: The synthetic nature of the dataset resulted in random and inconsistent values. Consistency was simulated through approximations and adjustments, which would not be necessary in a real-world dataset.
\end{itemize}
...
The \texttt{2\_Pulizia\_CSV\_GDP.py} script was employed to rescale World Bank GDP datasets to a consistent ground. 
The time period was filtered for values between 2020-2024, matching the range of the Online Sales Dataset. \\
Data quality improvement involved systematic handling of missing values:
\begin{itemize}
...
\begin{enumerate}
    ...
    The Discount column was normalized between 0 and 1.\\
    Text standardization corrected common typos and ensured consistent formatting for InvoiceNo and StockCode values.
    A significant enhancement was creating realistic multi-item orders.
    The original dataset contained mostly single-item transactions due to its synthetic nature so we grouped related items 
    into single multi-article orders
    ...
    The destination countries of the customers in the Online Sales Dataset (OSD) were matched with the countries in the GDP CSV file, using their respective ISO 3166-1 alpha-3 codes. 
    This approach was adopted to save space and simplify representation.
    Wherever economic data were missing, country-specific interpolation was applied where possible, and after that, the gaps that would still be left were filled by the median of the whole set of data for the world.
\end{enumerate}
\textbf{Final Dataset Quality}
The cleaning process transformed the original 49,782 records into a refined dataset of 43,434 records with significantly improved data quality.
...
The geographical situation is addressed by the \texttt{Customer}-\texttt{Country} relationship, and \texttt{Shipment} and \texttt{Payment} specify logistics and payment processes, respectively.
...
\textbf{Step 1 - initial attribute tree}: all the raw dataset attributes were mapped without filters, giving a complete but rough idea of the data available. 
...
\textbf{Step 2 - pruning and grafting}: Attributes such as \texttt{InvoiceNo}, \texttt{StockCode}, and \texttt{CustomerID} were grafted into the tree to enhance its structure.
...
\textbf{Step 3 - final version}: the tree contains only the attributes relevant for analytical purposes, already grouped according to the dimensions planned for the future DW.
...
The use of hierarchies, particularly within Product and Date dimensions, facilitates drill-down and roll-up operations.
Furthermore, the inclusion of macroeconomic variables as a dimension allows analysis to go beyond transactional data in order to gain perspective on how regional economic trends relate to consumer activity.
...
The Star Schema centralizes the analytical model around the \texttt{Sale} fact table.
The \texttt{Dim\_Junk} dimension groups low-cardinality attributes (e.g., \texttt{Return\_Status}, \texttt{Sales\_Channel}, \texttt{Payment\_Method}) into a compact structure.
