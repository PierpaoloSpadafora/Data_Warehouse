...
TEST_OUTPUT_CSV = "./TEST_READY_OSD_plus_GDP.csv"
...
# 1) Salva il file completo con separatore semicolon
merged.to_csv(OUTPUT_CSV, index=False, sep=';', float_format="%.2f")
print(f"File completo creato: {OUTPUT_CSV}")

# 2) Crea una copia con solo 200 record per i test
test_data = merged.head(200)
test_data.to_csv(TEST_OUTPUT_CSV, index=False, sep=';', float_format="%.2f")
print(f"File di test creato: {TEST_OUTPUT_CSV}")
